package com.dou.netty.websocket.nettywebsocket.handler;
...
import com.dou.netty.websocket.nettywebsocket.utils.HttpUtil;
...
public class WebSocketHandler extends SimpleChannelInboundHandler<TextWebSocketFrame> {
    ...
    public void handleHttpRequest(ChannelHandlerContext ctx, FullHttpRequest request)
        ...
        if (null != request) {
            if (request instanceof HttpRequest) {
                ...
                // 如果是websocket请求就握手升级
                if (request.uri().contains(webSocketPath)) {
    ...
    public void handlerWebSocketFrame(ChannelHandlerContext ctx, WebSocketFrame frame) throws Exception {
        ...
        if (frame instanceof TextWebSocketFrame) {
            ...
            String senderId = messageReq.getSenderId();
            String receiverId = messageReq.getReceiverId();
            //发送者id为空
            if (StringUtils.isBlank(messageReq.getSenderId())) {
                messageResp.setMsgEntity("发送用户不存在!");
                ctx.channel().writeAndFlush(new TextWebSocketFrame(JSON.toJSONString(messageResp)));
                return;
            }
            ...
            // 给receiverId发送消息
            if (exist(receiverId)) {
                getChannel(receiverId).writeAndFlush(new TextWebSocketFrame(JSON.toJSONString(messageResp)));
            }
        }
     ...
     * @return
     */
    private void bindUserId(ChannelHandlerContext ctx, FullHttpRequest request) {
        ...
        if (null == origin) {
            ...
            String userId = HttpUtil.getParamFromUri(uri, "userId");
            if (StringUtils.isNotBlank(userId)) {
                log.info("打开页面，绑定的userId: {}", userId);
                bind(ctx, userId);
            }
